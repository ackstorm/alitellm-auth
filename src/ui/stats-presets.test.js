// stats-presets.test.js — vitest unit suite for the PURE preset->date-range +
// capability->render-mode helpers.
//
// Mirrors the router.test.js / format.test.js style (describe/it/expect, default
// node env, no jsdom) because stats-presets.js touches no window/document/fetch
...
function inclusiveSpanDays(start, end) {
  ...
  it("NO preset ever returns a span greater than MAX_RANGE_DAYS, across many now values", () => {
    ...
    for (const now of probes) {
      for (const preset of PRESETS) {
        ...
        expect(Date.parse(`${start}T00:00:00Z`)).toBeLessThanOrEqual(
