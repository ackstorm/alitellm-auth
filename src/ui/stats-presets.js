// stats-presets.js — PURE preset->date-range + capability->render-mode helpers
// for the alitellm-auth Usage & Spend page (#/stats).
//
// This module is the ONE place all stats date arithmetic lives (UI-SPEC §"Left
// to the planner": keep date math out of the components). It mirrors the
// router.js / format.js discipline:
...
// Format a Date's UTC parts as "YYYY-MM-DD" (tz-stable, mirrors format.js:90-92
...
function inclusiveSpan(startDate, endDate) {
...
function clampRange(startDate, endDate) {
  if (inclusiveSpan(startDate, endDate) > MAX_RANGE_DAYS) {
    const clampedStart = new Date(
      endDate.getTime() - (MAX_RANGE_DAYS - 1) * MS_PER_DAY,
    );
...
export function capabilityRenderMode(capabilities, figureKey, hasData) {
