...
"""Internal endpoints for the Go authorization proxy.

`POST /api/internal/front-key {"sub"}` returns the LiteLLM key for a user.
The key is minted once under a per-user lock and kept Fernet-encrypted in the
AS store. Before a stored key is returned, LiteLLM is asked whether it still
recognizes it; a revoked or expired key is discarded and re-minted.
"""
...
async def front_key(request: Request) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse({"key": minted["key"]})
    finally:
